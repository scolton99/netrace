...
public class PingTask extends AsyncTask<Void, PingResult, List<PingResult>> {
    @NonNull private String specified_host;
    @NonNull private int count;
    @NonNull private int timeout_ms;
    @NonNull private WeakReference<Fragment> pingFragment;
    ...
    protected List<PingResult> doInBackground(Void... voids) {
        try {
            ...
            this.host = (Inet4Address) ia;
            this.canonical_host = this.host.getCanonicalHostName();
        ...
        for (int i = 0; i < this.count; ) {
            ...
            try {
                String command = String.format("ping -c 1 -W %.2f %s", this.timeout_ms / 1000.00f, this.host.getHostAddress());
                Log.w("PING", "Command: " + command);
